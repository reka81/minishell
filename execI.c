...
/*   Updated: 2024/06/06 17:20:09 by mettalbi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_env *environment, t_exec1 *var)
{
	...
	if (!ft_strcmp(final_linked->value[0], "export"))
		(export(final_linked, environment, var, var->exit_status), exit(*var->exit_status));
		...
		char **env, t_env *environment)
{
	...
	while (var->a > 1)
	{
		...
		var->pid_tab[var->i] = execute_cmds(final_linked, env, environment, var->exit_status);
