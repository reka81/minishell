...
/*   Updated: 2024/05/10 23:44:26 by mettalbi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rest_of_expanding3(t_stack *a, t_counter *count_num,
			t_exp *expander, int exit_status)
{
	char	*str;
			...
			t_exp *expander, char *str2)
{
	if (a->value[count_num->i] == '$')
	{
		...
		while (a->value[count_num->i])
		{
			...
			count_num->j = 0;
			rest_of_expanding3(a, count_num, expander, expander->exit_status);
			count_num->j = ft_strlen1(str2);
			count_num->j -= 1;
...
void	expanding(t_stack *a, int exit_status, t_env *environment)
{
	...
	while (a)
	{
		...
		if ((a->type == 1 || a->type == 0) && a->should_be_exp != 1)
		{
			...
			str2[count_num->i] = '\0';
			rest_of_expanding6(count_num, a, expander, str2);
		}
		a = a->next;
	}
}
